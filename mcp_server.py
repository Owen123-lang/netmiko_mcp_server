...
from tools.get_device_status import (
    get_device_status,
    get_device_uptime,
    get_memory_cpu_usage,
)
from tools.get_running_config import (
    get_running_config,
    get_interface_config,
    get_startup_config,
)
from tools.configure_interface import (
    configure_interface,
    configure_default_gateway,
    configure_dns,
)
from tools.configure_ospf import (
    configure_ospf,
    ...
    try:
        ...
        # Configuration tools
            ...
            if not all([device_name, interface_name, ip_address, subnet_mask]):
                ...
                result = configure_interface(
                    device_name, interface_name, ip_address, subnet_mask, description
                )
            ...
            if not all([device_name, process_id, networks]):
                ...
                result = configure_ospf(
                    device_name, process_id, networks, default_route
                )
        ...
        # Validation tools
            ...
            if not all([device_name, interface_name]):
                ...
                result = validate_interface_config(
                    device_name, interface_name, expected_ip
                )
        ...
        # Basic configuration tools
            ...
            if not all([device_name, interface_name, description]):
                ...
                result = configure_interface_description(
                    device_name, interface_name, description
                )
            ...
            if not all([device_name, loopback_number is not None, ip_address]):
                ...
                result = create_loopback(
                    device_name, loopback_number, ip_address, description
                )
        ...
        # Bootstrap tools
            ...
            if not all([target_ip, target_hostname]):
                result = {
                    "success": False,
                    "error": "target_ip and target_hostname are required",
                }
            else:
                result = bootstrap_router_ssh(
                    jumphost_device, target_ip, target_hostname, username, password
                )
