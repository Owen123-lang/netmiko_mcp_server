...
def execute_single_command(
    device_config: Dict[str, str], command: str
) -> Optional[str]:
    """
...
def get_device_config(device_name: str) -> Dict[str, any]:
    ...
    if device_name == "R2":
        ...
        if (
            jumphost_key not in _jumphost_connections
            or not _jumphost_connections[jumphost_key]
        ):
            ...
            logger.info("Reusing existing jumphost connection")
        ...
        try:
            ...
            logger.info(
                f"Attempting SSH channel: R1 → R2 ({dest_addr[0]}:{dest_addr[1]})"
            )
            ...
            try:
                ...
                ssh_command = (
                    f"ssh -l {device_config['username']} {device_config['host']}"
                )
